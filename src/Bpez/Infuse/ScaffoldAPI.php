...
trait ScaffoldAPI {
  ...
  {
    $base = 'hstore(array("attributes" => array("latitude", "longitude", "zoom"), ...));';

      if (!is_array($columns)) 
         throw new ScaffoldConfigurationException($base.' Must be passed in as an array.');
      
      foreach ($columns as $column => $keys) {
         if (!array_key_exists($column, $this->columns) && !is_array($keys)) { 
            throw new ScaffoldConfigurationException($base.'  Column '.$column.' doesn\'t exist or value is not of type array.');
         }
      }
